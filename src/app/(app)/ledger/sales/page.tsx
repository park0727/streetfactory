...
import { db } from "@/db";
import { partners, salesLines, salesOrders } from "@/db/schema";
...
export default async function SalesLedgerPage({ searchParams }: PageProps<"/ledger/sales">) {
  ...
  const c: SQL[] = [gte(salesOrders.docDate, from), lte(salesOrders.docDate, to)];
  if (partnerId) c.push(eq(salesOrders.partnerId, partnerId));
  if (q) c.push(or(ilike(salesOrders.docNo, `%${q}%`), ilike(partners.name, `%${q}%`))!);
  const where = and(...c);
  ...
  const agg = db
    .select({
      ...
      amount: sql<number>`sum(${salesLines.qty} * ${salesLines.unitPrice})::numeric`.as("amount"),
      profit: sql<number>`sum(${salesLines.qty} * (${salesLines.unitPrice} - ${salesLines.unitCost}))::numeric`.as("profit"),
    })
    .from(salesLines)
  ...
  const [rows, [tot], plist] = await Promise.all([
    base.orderBy(desc(salesOrders.docDate), desc(salesOrders.id)).limit(PAGE_SIZE).offset((page - 1) * PAGE_SIZE),
    db
      .select({ n: sql<number>`count(*)::int`, amount: sql<number>`coalesce(sum(${agg.amount}), 0)::numeric`, profit: sql<number>`coalesce(sum(${agg.profit}), 0)::numeric` })
      .from(salesOrders)
      .innerJoin(partners, eq(partners.id, salesOrders.partnerId))
      .leftJoin(agg, eq(agg.orderId, salesOrders.id))
      .where(where),
  ...
  return (
    <>
      <PageHeader title="판매 원장" description="모든 판매/출고 전표. 전표를 열면 라인별 단가·원가 스냅샷을 볼 수 있습니다." />
      <LedgerToolbar partners={plist} from={from} to={to} />
      <p className="mt-3 mb-3 text-[13px] text-steel">
        {from} ~ {to} · 전표 <b className="text-foreground">{num(tot.n)}</b>건 · 매출 <b className="text-foreground">{krw(tot.amount)}</b> · 이익 <b className="text-foreground">{krw(tot.profit)}</b>
      </p>
              ...
              return (
                <TableRow key={r.id}>
                  <TableCell className="code">
                    <Link href={`/ledger/sales/${r.id}`} className="hover:underline">{r.docNo}</Link>
                  </TableCell>
