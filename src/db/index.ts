 ...
 * - 개발(Node): DATABASE_URL 로, 역시 요청마다 새 클라이언트. 전역 1개를 오래 쓰면 풀러가 끊은 소켓을 재사용하다
 *   60초 멈추는 일이 있었다. idle_timeout 으로 요청이 끝난 연결은 곧 닫힌다.
...
function createDb(url: string) {
  const client = postgres(url, { prepare: false, max: 5, idle_timeout: 10, connect_timeout: 10, fetch_types: false });
